...
def item_context(
    ...
    depth = str(item.metadata.get("summary_depth") or "brief")
    length_target = str(item.metadata.get("summary_length_zh") or "100-180字")
    editorial_contract = (
        "Provide a deep interpretation with background, the key change, affected "
        "groups, opportunities/risks, and practical meaning for AI product managers "
        "or FDE work."
        if depth == "deep"
        else "Provide a compact factual brief focused on the change, importance, and one actionable takeaway."
    )
    return f"""# Item

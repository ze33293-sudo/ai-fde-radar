...
def redact_url(url: str) -> str:
    ...
    path = parts.path
    if host in {"open.feishu.cn", "open.larksuite.com"} and "/hook/" in path:
        prefix = path.partition("/hook/")[0]
        path = f"{prefix}/hook/<redacted>"
...
class WebhookNotifier:
    ...
    def _validate_url(self, url: str) -> str:
        ...
        try:
            ...
            if str(e).startswith("Invalid URL:"):
                raise ValueError(
                    "Webhook URL is structurally invalid "
                    f"(env var '{self.config.url_env}')"
                ) from e
            raise ValueError(f"Webhook URL is invalid: {e}") from e
        except httpx.InvalidURL as e:
            raise ValueError(
                "Webhook URL is structurally invalid "
                f"(env var '{self.config.url_env}', error={type(e).__name__})"
            ) from e
        return url
    ...
    def _build_feishu_collapsible_overview(
        ...
        if lang == "zh":
            if item_count == 0:
                return (
                    f"# AI FDE Radar - {date}\n\n"
                    f"> 今日暂无高价值更新。已检查 {all_items_count} 条内容，"
                    "没有条目达到 7/10 的质量门槛。"
                )
            return (
                f"# Horizon 每日速递 - {date}\n\n"
    ...
    async def notify(self, variables: dict) -> WebhookDeliveryResult:
        ...
        try:
            ...
            safe_detail = str(e)
            self.console.print(
                f"[red]Webhook URL is invalid: {safe_detail}[/red]"
            )
            logger.error(
                "Webhook URL rejected: URL=%s, reason=%s", safe_url, safe_detail
            )
            return WebhookDeliveryResult(
            ...
            self.console.print("[red]Webhook URL is invalid.[/red]")
            logger.error("Webhook URL invalid; env var: %s", self.config.url_env)
            return WebhookDeliveryResult(
                WebhookDeliveryStatus.NETWORK_FAILURE,
            ...
            self.console.print(
                "[red]Webhook connection failed.[/red]"
            )
            logger.error("Webhook connection failed: URL=%s", safe_url)
            return WebhookDeliveryResult(
                WebhookDeliveryStatus.NETWORK_FAILURE,
                detail="webhook connection failed",
                error_type="connect",
            )
        except httpx.TimeoutException as e:
            self.console.print(
                "[red]Webhook request timed out.[/red]"
            )
            logger.error("Webhook timeout: URL=%s", safe_url)
            return WebhookDeliveryResult(
                WebhookDeliveryStatus.NETWORK_FAILURE,
            ...
            self.console.print(
                f"[red]Webhook call failed unexpectedly: {type(e).__name__}[/red]"
            )
            logger.error("Webhook unexpected error: URL=%s, type=%s", safe_url, type(e).__name__)
            return WebhookDeliveryResult(
                WebhookDeliveryStatus.INTERNAL_FAILURE,
                detail=f"unexpected webhook error: {type(e).__name__}",
    ...
    async def send_daily_summary(
        ...
        for message in messages:
            ...
            for attempt in range(2):
                ...
                if attempt == 0:
                    self.console.print(
                        "[yellow]Webhook delivery failed; retrying once.[/yellow]"
                    )

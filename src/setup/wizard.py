...
def main():
    ...
    try:
        ...
        if args.data_dir == "data":
            presets_path_arg = "data/presets.json"
        elif not presets_path.exists():
            presets_path_arg = "data/presets.json"
        else:
            presets_path_arg = str(presets_path)
        presets = load_presets(presets_path=presets_path_arg, prefer_api=True)
...
def _count_sources(config: Config) -> int:
    ...
    if config.sources.gdelt and config.sources.gdelt.enabled:
        count += 1
    if any(query.enabled for query in config.sources.google_news_queries()):
        count += 1
    return count

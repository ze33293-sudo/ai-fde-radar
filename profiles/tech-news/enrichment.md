...
- `try_today` — localized title “今天怎么试”. This block is optional. Include it only when the evidence supports one safe 15–30 minute experiment with a visible output or pass/fail check. Prefer a tiny evaluation set, workflow sketch, prompt comparison, failure log, or repository demo, and omit the block when no honest experiment follows from the source.
